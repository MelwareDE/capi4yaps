...
static void n0(capi_contr * card, capi_ncci * ncci)
{
	...
	capi_fill_DISCONNECT_REQ(&gcmsg,
				 ctx->appid,
				 card->msgid++,
				 ncci->plcip->plci,
...
static int handle_charge_info(capi_connection *plcip, _cmsg *cmsg)
{
	...
	if ((cmsg->InfoNumber & 0x4000) && p[0] == 4) {
		unsigned char *pp = &cmsg->InfoElement[1];
		charge |= ((unsigned)pp[0]);
		charge |= ((unsigned)pp[1]) << 8;
		charge |= ((unsigned)pp[2]) << 16;
		charge |= ((unsigned)pp[3]) << 24;
		if (cb->chargeinfo) {
